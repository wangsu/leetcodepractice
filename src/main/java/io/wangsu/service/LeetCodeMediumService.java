...
public class LeetCodeMediumService {
    ...
    public int totalNQueens(int n) {
        ...
        totalNQueensHelper(0,cols,d1,d2,n);
        return count;
    }

    private void totalNQueensHelper(int row, boolean[] cols, boolean[] d1, boolean[] d2, int n){
        if(row==n){
            count++;
        }
        for(int col=0;col<n;col++){
            ...
            d1[id1]=true;
            d2[id2]=true;
            totalNQueensHelper(row+1,cols,d1,d2,n);
            cols[col]=false;
            d1[id1]=false;
            d2[id2]=false;
        }
    ...
    private ListNode reverse(ListNode pre,ListNode nextStart){
